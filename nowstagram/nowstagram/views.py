...
# 更多判断'0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
...
def reg():
    ...
    salt = '.'.join(random.sample('0123456789abcdefghigABCDEFGHI', 10))
    m = hashlib.md5()
...
def index_image(page, per_page):
    paginate = Image.query.order_by_by(db.desc(Image.id)).paginate(page=page, per_page=per_page, error_out=False)
    map = {'has_next': paginate.has_next}
    images = []
    for image in paginate.items:
        comments = []
